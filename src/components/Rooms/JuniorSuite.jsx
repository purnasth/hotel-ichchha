      ...
      <section className="bg-bg-gold-light">
        <div className="container mb-24">
          <div className="flexCenter flex-col w-3/4 mx-auto mb-32">
            <h3 className="text-navy font-medium">{juniorSuite.title}</h3>
            <h4 className="text-3xl text-center">{juniorSuite.subtitle}</h4>
            <p className="text-base text-center">{juniorSuite.description}</p>
          </div>

          <div className="flex justify-between">
            <div className="w-1/2 max-w-lg">
              <h5 className="text-3xl leading-loose pb-4">
                {juniorSuite.overview[0].title}
              </h5>
              <p className="pb-4">{juniorSuite.overview[0].content}</p>

              {juniorSuite.policies.map((policyCategory) => (
              ...
              <div className="flex items-start gap-20">
                <ul className="flex items-start justify-center flex-col gap-5">
                  {juniorSuite.amenities
                    .slice(0, Math.ceil(juniorSuite.amenities.length / 2))
                    .map((amenity) => (
                ...
                <ul className="flex items-start justify-center flex-col gap-5">
                  {juniorSuite.amenities
                    .slice(Math.ceil(juniorSuite.amenities.length / 2))
                    .map((amenity) => (
                      <li key={amenity} className="flex items-center gap-4">
                        <HiArrowLongRight />
                        {amenity}
            ...
            <RoomDetailsWithBooking
              roomDetails={juniorSuite.roomDetails}
